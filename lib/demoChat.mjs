  ...
  return async function handle(request) {
    ...
    try {
      ...
      if (request.method === "GET") {
        ...
        return json({ available: true, limit: GUEST_TURNS, remaining: Math.max(0, Math.min(GUEST_TURNS - Number(counts[0] || 0), IP_TURNS - Number(counts[1] || 0))), siteKey: config.siteKey }, 200, identity.cookie);
      }
