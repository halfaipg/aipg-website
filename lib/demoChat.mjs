...
import { GUEST_TURNS, IP_TURNS, MAX_MESSAGES, MAX_CONTEXT_BYTES } from "./demoChatPolicy.mjs";
import { IMAGE_MODELS, IMAGE_LIMIT, IMAGE_IP_LIMIT, IMAGE_TOOL, IMAGE_RESERVE_LUA, imagePrompt, safeImageUrl } from "./demoImagePolicy.mjs";
...
export function demoConfig(env = process.env) {
  ...
  // Vercel's managed REDIS_URL is a TCP URL; the HTTP limiter needs its REST pair.
  const managedRedis = env.DEMO_KV_REST_API_URL !== undefined || env.DEMO_KV_REST_API_TOKEN !== undefined;
  const redis = new URL(managedRedis ? env.DEMO_KV_REST_API_URL : env.DEMO_REDIS_URL);
  const redisToken = managedRedis ? env.DEMO_KV_REST_API_TOKEN : env.DEMO_REDIS_TOKEN;
  ...
  if ((!local && (origin.protocol !== "https:" || env.VERCEL !== "1")) || origin.origin !== env.DEMO_CHAT_ORIGIN ||
      ...
      !env.DEMO_GRID_KEY || !redisToken || (env.DEMO_COOKIE_SECRET || "").length < 32 ||
  ...
  const images = env.DEMO_IMAGES_ENABLED === "1";
  const imageModel = env.DEMO_IMAGE_MODEL || IMAGE_MODELS[0];
  if (images && !IMAGE_MODELS.includes(imageModel)) throw new Error("Invalid demo image model");
  return { origin: origin.origin, hostname: origin.hostname, local, redis: redis.origin,
    key: env.DEMO_GRID_KEY, redisToken, secret: env.DEMO_COOKIE_SECRET,
    siteKey: env.DEMO_TURNSTILE_SITE_KEY, turnstileSecret: env.DEMO_TURNSTILE_SECRET, perRequest, daily, images, imageModel };
}

export async function readBounded(stream, max = 16000, signal) {
  if (!stream) return "";
...
export function validateMessages(body) {
  if (!body || Object.keys(body).some(k => !["messages", "token"].includes(k)) ||
      ...
      !Array.isArray(body.messages) || body.messages.length < 1 || body.messages.length > MAX_MESSAGES || body.messages.length % 2 !== 1) {
  ...
  if (bytes > MAX_CONTEXT_BYTES) throw new DemoError(413, "too_large", "This conversation is too long. Continue in Chat.");
  return messages;
}
...
if used >= ${GUEST_TURNS} or ipused >= ${IP_TURNS} then return {'quota', math.max(0, math.min(${GUEST_TURNS}-used, ${IP_TURNS}-ipused))} end
...
if redis.call('ZCARD', KEYS[4]) >= 4 or redis.call('EXISTS', KEYS[5]) == 1 then return {'busy', math.min(${GUEST_TURNS}-used, ${IP_TURNS}-ipused)} end
...
return {'ok', math.min(${GUEST_TURNS - 1}-used, ${IP_TURNS - 1}-ipused)}
`;
...
export function createDemoHandler({ env = process.env, fetcher = fetch, now = Date.now } = {}) {
  return async function handle(request) {
    ...
    try {
      ...
      const imageGuestKey = `${guestKey}:images`;
      const imageIpKey = `${ipKey}:images`;
      if (request.method === "GET") {
        ...
        let imageStatus;
        if (config.images) {
          const counts = await command(["MGET", imageGuestKey, imageIpKey]);
          if (!Array.isArray(counts) || counts.length !== 2 || counts.some(n => n !== null && (!/^\d+$/.test(String(n)) || !Number.isSafeInteger(Number(n))))) throw new Error("Image limiter contract changed");
          imageStatus = { limit: IMAGE_LIMIT, remaining: Math.max(0, Math.min(IMAGE_LIMIT - Number(counts[0] || 0), IMAGE_IP_LIMIT - Number(counts[1] || 0))), model: config.imageModel };
        }
        return json({ available: true, limit: GUEST_TURNS, remaining: Math.max(0, Math.min(GUEST_TURNS - Number(counts[0] || 0), IP_TURNS - Number(counts[1] || 0))), siteKey: config.siteKey, ...(imageStatus ? { images: imageStatus } : {}) }, 200, identity.cookie);
      }
      ...
      if (!Array.isArray(reserved) || reserved.length !== 2 || !["ok", "quota", "busy", "budget"].includes(reserved[0]) || !Number.isInteger(reserved[1]) || reserved[1] < 0 || reserved[1] > GUEST_TURNS) throw new Error("Limiter contract changed");
      ...
      // Require Core's authenticated, model-independent direct-service policy.
      const policy = balance.service_budget;
      if (!credits.ok || balance.charging_enabled !== true || !["on", "allowlist"].includes(balance.charging_mode)
        || policy?.version !== 1 || policy.all_models_charged !== true
        || !Number.isSafeInteger(policy.per_request_micro) || policy.per_request_micro <= 0 || policy.per_request_micro > config.perRequest
        || !Number.isSafeInteger(policy.daily_micro) || policy.daily_micro <= 0 || policy.daily_micro > config.daily) {
        throw new Error("Sponsored auto charging is not active");
      }
      const upstream = await fetcher(`${CORE}/v1/chat/completions`, {
        ...
        body: JSON.stringify({ model: "auto", stream: true, max_tokens: MAX_OUTPUT, reasoning_effort: "low", messages: [
          { role: "system", content: "Answer the user's question directly. Keep replies concise, usually under 150 words. Do not invent current Grid availability, prices, or account details. " + (config.images ? "You may call generate_image once when the user explicitly requests a new image. For normal chat, answer normally without tools. Never claim you generated or edited an image yourself. No browsing, uploads, image inspection, editing, video or other tools. Only the tool result can establish generation succeeded." : "You have no tools or browsing.") }, ...messages,
        ], ...(config.images ? { tools: [IMAGE_TOOL], tool_choice: "auto", parallel_tool_calls: false } : {}) }),
      });
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          try {
            ...
            for await (const event of chatEvents(upstream.body, { allowImageTool: config.images })) {
              if (event.type !== "image_tool") { send(event); continue; }
              const prompt = imagePrompt(event.call);
              signal.throwIfAborted();
              // No automatic fallback/retry: a timeout may already have created a paid job.
              const modelsResponse = await fetcher(`${CORE}/v1/status/models`, { signal, cache: "no-store", redirect: "error" });
              const models = JSON.parse(await readBounded(modelsResponse.body, 128000, signal));
              if (!modelsResponse.ok || !Array.isArray(models) || !models.some(m => m.name === config.imageModel && m.type === "image" && m.count > 0)) {
                throw new DemoError(503, "image_unavailable", "The image model is offline right now. You can keep chatting or open the image studio.");
              }
              const imageReservation = await command(["EVAL", IMAGE_RESERVE_LUA, "4", imageGuestKey, imageIpKey, `${prefix}:budget`, activeIp,
                String(config.perRequest), String(config.daily), String(identity.ttl + 3600), id]);
              if (!Array.isArray(imageReservation) || imageReservation.length !== 2 || !["ok", "quota", "budget", "expired"].includes(imageReservation[0]) || !Number.isInteger(imageReservation[1]) || imageReservation[1] < 0 || imageReservation[1] >= IMAGE_LIMIT) throw new Error("Image limiter contract changed");
              if (imageReservation[0] !== "ok") {
                if (imageReservation[0] === "quota") send({ type: "image_quota", remaining: 0 });
                throw new DemoError(429, "image_limit", imageReservation[0] === "quota" ? "Your free image allowance is used. You can keep chatting or open the image studio." : "Image generation is unavailable right now. Please try the image studio.");
              }
              signal.throwIfAborted();
              send({ type: "image_start", model: config.imageModel, remaining: imageReservation[1] });
              const result = await fetcher(`${CORE}/v1/images/generations`, { method: "POST", headers, redirect: "error", signal,
                body: JSON.stringify({ model: config.imageModel, prompt, n: 1, size: "1024x1024", response_format: "url", output_format: "webp" }) });
              if (!result.ok) { await result.body?.cancel(); throw new DemoError(503, "image_failed", "Image generation did not complete. It has not been retried automatically. You can keep chatting or open the image studio."); }
              const data = JSON.parse(await readBounded(result.body, 16000, signal));
              const url = data.data?.length === 1 ? safeImageUrl(data.data[0].url) : null;
              if (!url || data.grid?.model !== config.imageModel) throw new Error("Invalid image result");
              send({ type: "image", url, prompt, model: config.imageModel });
              send({ type: "done", stats: responseStats({ grid: data.grid }) });
            }
          } catch (error) {
            if (!signal.aborted) send({ type: "error", message: error instanceof DemoError ? error.message : "The response was interrupted. Continue in Chat or try again." });
          } finally {
...
export async function* chatEvents(stream, { allowImageTool = false } = {}) {
  ...
  let tool = null;
  const protocolPrefix = "<\uff5cDSML\uff5c";
  let pendingText = ""; let protocolText = false;
  // Hold only a possible reserved-marker prefix, so split tokens cannot leak.
  const visibleText = (fragment, final = false) => {
    if (protocolText) return "";
    pendingText += fragment;
    const marker = pendingText.indexOf(protocolPrefix);
    if (marker >= 0) {
      const text = pendingText.slice(0, marker);
      pendingText = ""; protocolText = true;
      return text;
    }
    let keep = 0;
    if (!final) {
      for (let size = 1; size < protocolPrefix.length; size++) {
        if (pendingText.endsWith(protocolPrefix.slice(0, size))) keep = size;
      }
    }
    const text = pendingText.slice(0, pendingText.length - keep);
  ...
  try {
    while (true) {
      ...
      while ((index = buffer.indexOf("\n\n")) >= 0) {
        ...
        if (data === "[DONE]") {
          if (tool) {
            if (!allowImageTool || reason !== "tool_calls") throw new Error("Incomplete image tool");
            imagePrompt(tool);
            const tail = visibleText("", true);
            if (tail) yield { type: "delta", text: tail };
            yield { type: "image_tool", call: tool }; return;
          }
          if (!textLength || !finished || protocolText) throw new Error("Incomplete response");
          const tail = visibleText("", true);
          if (tail) yield { type: "delta", text: tail };
        ...
        if (calls != null && !Array.isArray(calls)) throw new Error("Unsupported tool calls");
        // Some backends include an empty list on ordinary text deltas.
        if (calls?.length) {
          if (!allowImageTool || calls.length !== 1 || calls[0]?.index !== 0) throw new Error("Unsupported tool calls");
          const call = calls[0];
          if (call.type !== undefined && call.type !== "function") throw new Error("Unsupported tool type");
          tool ||= { name: "", arguments: "" };
          for (const key of ["name", "arguments"]) {
            const fragment = call.function?.[key];
            if (fragment !== undefined) {
              if (typeof fragment !== "string") throw new Error("Invalid tool fragment");
              tool[key] += fragment;
            }
          }
          if (tool.name.length > 64 || tool.arguments.length > 8192) throw new Error("Tool too large");
        }
        ...
        if (typeof choice?.delta?.content === "string") {
          ...
          const text = visibleText(choice.delta.content);
