...
test.describe('/validate smoke', () => {
  test('states the preview trust boundary and renders verified preview downloads', async ({ page }) => {
    ...
    await expect(page.getByText(/preview\.14 keeps setup and start as separate actions/)).toBeVisible();
    ...
    await expect(page.getByRole('link', { name: 'Download Linux installer' })).toHaveAttribute(
      'href',
      /releases\/download\/v0\.1\.0-preview\.14\/install-validator\.sh$/,
    );
    await expect(page.getByRole('link', { name: 'Linux x64' })).toHaveAttribute(
      'href',
      /releases\/download\/v0\.1\.0-preview\.14\/aipg-validator-linux-x64\.zip$/,
    );
    await expect(page.getByRole('link', { name: 'Windows x64' })).toHaveAttribute(
      'href',
      /releases\/download\/v0\.1\.0-preview\.14\/aipg-validator-windows-x64\.zip$/,
    );
    await expect(
      page.getByText('docker pull ghcr.io/aipowergrid/validator:v0.1.0-preview.14'),
    ).toBeVisible();
