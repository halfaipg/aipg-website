...
test.describe("/run smoke", () => {
  test("renders the release-gated worker download surface", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Text worker" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Media manager" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Linux", exact: true }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Windows", exact: true }).first(),
    ).toBeVisible();
    await expect(
...
test.describe("/run mobile smoke", () => {
  ...
  test("keeps the download path readable without horizontal overflow", async ({
    page,
  }) => {
    ...
    await page
      ...
      .click();
    await page
      .getByLabel("Accelerator type", { exact: true })
      .selectOption("apple");
    await expect(
      page.getByRole("heading", {
    ...
    await page.screenshot({
      path: "test-results/run-mobile.png",
      fullPage: true,
    });
  });
});
...
test.describe("/validate smoke", () => {
  test("states the preview trust boundary and renders a gated release path", async ({
    ...
    await expect(
      page.getByText(/no validator rewards, staking, slashing/i),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: /Create validator key/i }),
    ).toBeVisible();
    await expect(
      page.getByText(
...
test.describe("/validate mobile smoke", () => {
  ...
  test("keeps validator onboarding readable without horizontal overflow", async ({
    ...
    expect(overflow).toBe(false);
    await page.screenshot({
      path: "test-results/validate-mobile.png",
      fullPage: true,
    });
  });
});
...
test.describe("/status smoke", () => {
  test("renders an honest aggregate status or feed-unavailable state", async ({
    page,
  }) => {
    const browserErrors: string[] = [];
    page.on("console", (message) => {
    ...
    const live = page.getByRole("heading", {
      name: /AI Power Grid is (operational|degraded)/i,
    });
    const unavailable = page.getByRole("heading", {
      name: "Live status feed unavailable",
    });
